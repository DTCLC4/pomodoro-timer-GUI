...
const App: React.FC = () => {
  ...
  const [isClicked, setIsClicked] = useState(false); // Trạng thái đã click

  const handleOpenModal = () => {
    setShowModal(true);
    setIsClicked(true); // Đặt state thành true khi click
  };

  const handleCloseModal = () => {
    setShowModal(false);
    setIsClicked(false); // Đặt state lại về false khi đóng Modal
  };
  ...
  return (
    <div className='container'>
      ...
      <Controls />
      <Timer />
      <button className={`pomodoro-app__preferences ${isClicked ? 'clicked' : ''}`} name="settings" title="show preferences pane" onClick={handleOpenModal}>
      ...
      <Settings show={showModal} handleClose={handleCloseModal} title="Custom Modal Title" />
    </div>
  )
}
